	...
	***********************************************/
	if(resp){
		...
		********************************************/
		...
		/********************************************
		** check the file existence in the server
		********************************************/
		if((rlen = recv(server_fd, &nresp, sizeof(nresp), 0)) < 0){
			perror("get resp | recv : ");
			exit(1);
		...
		*******************************************/
		if(resp){
			//write a vaild filename down in rec.list
			//fprintf(rfp, "%s\n", filename);
			printf("Downloading %s from the Server\n", filename);

			if((rlen = recv(server_fd, &nfsize, sizeof(nfsize), 0)) < 0){
			...
			/*************************************
			** set path and file open
			****************************************/
			sprintf(rec_path, "%s/%s", REC_DIR, filename);
			if( (fp = fopen(rec_path, "w")) == NULL){
				perror("file open fail | fopen : ");
				exit(1);
			}
			...
			while(cfsize < fsize){
				if((rlen = recv(server_fd, buf, sizeof(buf), 0)) < 0){
			...
			** Get file Complete
			****************************************/
			printf("Get file complete.\n");
			if(fp) fclose(fp);
#endif
		}else{
#if DEBUG
			printf("No file in the server : %s\n", filename);
#endif
		}	
	}
